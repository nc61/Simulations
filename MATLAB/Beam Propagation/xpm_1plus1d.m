...
% Initial envelope
phi_pump = exp(-((X - 0.5*L_space).^2./0.3^2 + (T - 0.5*L_time).^2./150.^2));
phi_probe = exp(-((X - 0.5*L_space).^2./0.1^2 + (T - 0.5*L_time).^2./150.^2));

% Linear propagation operator
D_pump = -(0.5*(-1i*0.1*Kx.^2 + 1i*1*OMEGA.^2)*0.5*dZ);
D_probe = -(0.5*(-1i*0.1*Kx.^2 + 1i*1*OMEGA.^2)*0.5*dZ);
