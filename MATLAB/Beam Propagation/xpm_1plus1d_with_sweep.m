...
[Kx, OMEGA] = meshgrid(kx_pump,omega_pump);
...
for jnd = 1:length(delays)
    ...
    % Initial envelope
    phi_pump = exp(-((X - 0.5*L_space).^2./5^2 + (T - delays(jnd)*L_time).^2./6.617.^2));
    phi_probe = exp(-((X - 0.5*L_space).^2./5^2 + (T - 0.5*L_time).^2./6.617.^2));
    
    % Linear propagation operator
    D_pump = -(0.5*(-1i*0*Kx.^2 - 20*1i*Kx + 1i*0*OMEGA.^2)*0.5*dZ);
    D_probe = -(0.5*(-1i*0*Kx.^2 + 1i*0*OMEGA.^2)*0.5*dZ);
    
    phi_out_pump = zeros(steps, N_time, N_space);
    phi_out_probe = zeros(steps, N_time, N_space);
    ...
    for ind = 1:steps
        P_pump(ind) = dX*dT*sum(sum(abs(phi_out_pump(ind,:,:)).^2));
        P_probe(ind) = dX*dT*sum(sum(abs(phi_out_probe(ind,:,:)).^2));
    end
...
hold on
surf(t,z,abs(squeeze(phi_out_probe(:,:,N_space/2+1))).^2)
hold off
...
(max(signal) - min(signal))/max(signal)
