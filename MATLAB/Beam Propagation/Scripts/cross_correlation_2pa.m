...
E_probe_initial = trapz(t_fs, abs(phi_probe).^2);

phi_probe_out = exp(-sample_thickness_mm*phi_pump.^2*alpha_2_mm_per_W);
...
for ind = 1:length(delay_fs)
    ...
    cross_correlation(ind) = trapz(t_fs, intensity_out);
end

norm_trans_method1 = cross_correlation/E_probe_initial;
normalized_transmission = 1 - 2*alpha_2_mm_per_W*pump_peak_power_W*sample_thickness_mm*(pump_pulsewidth_fs^2/(pump_pulsewidth_fs^2 + probe_pulsewidth_fs^2))*exp(-delay_fs.^2/(pump_pulsewidth_fs^2 + probe_pulsewidth_fs^2));
