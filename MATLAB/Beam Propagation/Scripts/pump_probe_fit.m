function Trans = pump_probe_fit(alpha_2_nd_cm_per_GW, pump_pulse_width_HW_einv_max_fs, probe_pulse_width_HW_einv_max_fs, pump_energy, sample_thickness_m, pump_spot_size_x_HW_einv2_max_m, pump_spot_size_y_HW_einv2_max_m, probe_spot_size_HW_einv2_max_m, t_d)
...
T_num = 200;                                     % Number of points in T and T_d
...
% R_p = 0;
I_0_p = 2*pump_energy/(pi^(3/2)*pump_spot_size_x_HW_einv2_max_m*pump_spot_size_y_HW_einv2_max_m*tau_pump) * (1-R_pump);   % Peak Irradiance of Pump [W/m^2]
...
T_d_term = abs(exp(-1/2*(T2 - T_d2 - rho - fit_parameters(3)*1e-15/tau_pump).^2/t^2)).^2;
