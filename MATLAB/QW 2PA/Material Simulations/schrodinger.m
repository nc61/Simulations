function [wavefunctions, energies, dz] = schrodinger(V, z, Meff, Eincrement)
...
%       value). Adjust Eincrement accordingly.
    ...
    % nearest values
    Meff_plus_half = 1/2*(Meff + [Meff(2:end) Meff(end)]);
    Meff_minus_half = 1/2*(Meff + [Meff(1) Meff(1:end-1)]);
...
for jnd = 1:length(E)
    ...
    for ind = 2:length(z)-1
        X(ind + 1) = Meff_plus_half(ind)*(((2*dz^2/hbar^2)*1.6e-19/1000*(V(ind) - E(jnd)) + 1/Meff_plus_half(ind) + 1/Meff_minus_half(ind))*X(ind) - X(ind - 1)/Meff_minus_half(ind));
    end
...
% for each solution
wavefunctions = zeros(length(crossings), length(z));
...
for lnd = 1:length(crossings)
    ...
    while (abs(X(end))/abs(max(X)) >= 1e-5 && (attempts < 100) || first_run)
        ...
        for ind = 2:length(z)-1
            X(ind + 1) = Meff_plus_half(ind)*(((2*dz^2/hbar^2)*1.6e-19/1000*(V(ind) - En) + 1/Meff_plus_half(ind) + 1/Meff_minus_half(ind))*X(ind) - X(ind - 1)/Meff_minus_half(ind));
        end
